...
export const Route = createFileRoute('/_app/notifications')({
  loader: async ({ context }) => {
    await context.queryClient.prefetchQuery(context.trpc.admin.notifications.list.queryOptions())
  },
...
function NotificationsLayout() {
  ...
  const isAddRoute = useMatch({ from: '/_app/notifications/add', shouldThrow: false })
  const notificationsQuery = useQuery(trpc.admin.notifications.list.queryOptions())
        ...
        <div className="w-80 border-r flex flex-col bg-card/50">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            {notifications.map((notification) => {
              const isActive = pathname.includes(`/notifications/${notification.id}`)
              const statusColor = {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {notification.priority === 'URGENT' && (
                          <span className={cn(
                            "text-xs px-2 py-0.5 rounded",
                            isActive ? "bg-primary-foreground/20" : "bg-red-100 text-red-800"
                          )}>
                            URGENT
                          </span>
                        )}
