  ...
  list: adminProcedure
    .input(
      z.object({
        ...
        type: z.enum(["ANNOUNCEMENT", "EVENT", "DONATION", "SYSTEM_ALERT", "OTHER"]).optional(),
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.groupId) conditions.push(eq(notifications.groupId, input.groupId));
  ...
  delete: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (notification.status === "DRAFT") {
        // Hard delete
        await db.delete(notificationRecipients).where(eq(notificationRecipients.notificationId, input.id));
