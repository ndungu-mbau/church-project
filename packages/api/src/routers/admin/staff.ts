...
export const adminStaffRouter = router({
  create: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Check if already staff in this church
      ...
      if (existingStaff) {
        throw new TRPCError({ code: "CONFLICT", message: "User is already staff in this church." });
      }

      // Update User Role (If upgrading? Or always set?). 
      // If user is "admin" somewhere else, changing role here affects global?
      // Assuming global role.
  ...
  update: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       if (!staffRecord) throw new TRPCError({ code: "NOT_FOUND", message: "Staff record not found" });
  ...
  delete: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       await db.delete(staff).where(and(
         ...
         eq(staff.churchId, ctx.session.churchId!)
       ));
